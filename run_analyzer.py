#!/usr/bin/env python3
import os
import pandas as pd

def main():
    print("=" * 50)
    print("Billboard Hot 100 Chart Analyzer")
    print("=" * 50)

    # Prompt for artist name
    artist_name = input("\nEnter artist name: ").strip()

    if not artist_name:
        print("❌ Error: Artist name cannot be empty!")
        return

    # Direct path with your username
    file_path = '/Users/prangonbarua/Desktop/hot100.csv'

    # Check if file exists
    if not os.path.exists(file_path):
        print(f"❌ Error: Data file not found at {file_path}")
        print("Please make sure hot100.csv is on your Desktop.")
        return

    print(f"\n🔍 Searching for '{artist_name}' in Billboard Hot 100 data...")

    # Load the data
    data = pd.read_csv(file_path, low_memory=False)

    # Check if all required columns are present
    required_cols = {'Date', 'Song', 'Artist', 'Rank'}
    if required_cols.issubset(data.columns):
        # Convert the 'Date' column to datetime
        data['Date'] = pd.to_datetime(data['Date'], errors='coerce')
        data = data.dropna(subset=['Date'])

        # Clean text fields
        data['Song'] = data['Song'].str.strip().str.lower()
        data['Artist'] = data['Artist'].str.strip().str.lower()

        # Filter by artist
        filtered_data = data[data['Artist'].str.contains(artist_name.lower(), na=False)]

        if not filtered_data.empty:
            # Create a 'Song (Artist)' column
            filtered_data = filtered_data.copy()
            filtered_data['Song_Artist'] = (
                filtered_data['Song'].str.title() + " (" + filtered_data['Artist'].str.title() + ")"
            )

            # Pivot table using actual dates from the data (not artificially generated)
            pivot_table = filtered_data.pivot_table(
                index='Date',
                columns='Song_Artist',
                values='Rank',
                aggfunc='first'
            )

            # Fill in missing weeks (only those present in the data)
            all_dates = pd.Series(data['Date'].unique())
            all_dates = all_dates[all_dates >= filtered_data['Date'].min()]
            pivot_table = pivot_table.reindex(pd.to_datetime(sorted(all_dates)))

            # Sort columns by first appearance
            first_appearance = filtered_data.groupby('Song_Artist')['Date'].min()
            sorted_columns = first_appearance.sort_values().index
            pivot_table = pivot_table[sorted_columns]

            # Format index as text
            pivot_table.index = pivot_table.index.strftime('%Y-%m-%d')

            # Safe filename for output
            safe_name = artist_name.title().replace(" ", "_")

            # Output file path to Desktop
            output_file = f'/Users/prangonbarua/Desktop/{safe_name}_Chart_History.xlsx'

            # Save to Excel (requires openpyxl installed)
            pivot_table.to_excel(output_file)

            print(f"\n✅ Success! Data for '{artist_name.title()}' saved to:")
            print(f"   {output_file}")
            print(f"\n📊 Found {len(sorted_columns)} songs with chart data")
        else:
            print(f"\n⚠️ No results found for artist: {artist_name}")
            print("   Try checking the spelling or try a different artist name.")
    else:
        print("❌ Error: Missing required columns in data file.")
        print("   Expected columns: Date, Song, Artist, Rank")

if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\n\n❌ Operation cancelled by user.")
    except Exception as e:
        print(f"\n❌ An error occurred: {str(e)}")
